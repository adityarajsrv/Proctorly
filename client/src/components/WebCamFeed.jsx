...
const WebCamFeed = ({
  ...
  // main processing
  useEffect(() => {
    if (!videoRef.current || !stream || !faceLandmarker) return;

    videoRef.current.srcObject = stream;

    const processFrame = async () => {
      if (!videoRef.current || interviewEnded) return;
      ...
      if (videoRef.current.readyState >= 2) {
        ...
        if (faces.length > 1) onProctorEvent?.("multiple-faces");
  ...
  }, [stream, faceLandmarker, objectModel, interviewEnded]);

  // periodic absence / looking-away checks
  useEffect(() => {
